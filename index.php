				...
				<div style="width: 100%;  border:3px solid white; padding: 2%; margin-top: 2%; ">
					...
					while($row = mysqli_fetch_assoc($cat_query_result)){ 
						...
						if (mysqli_num_rows($product_query_result)>0) { ?>
				          ...
				          <a class="dropdown-item text-light mt-2" href="#<?php echo $row['cat_name']; ?>"><?php echo $row['cat_name']; ?></a>
			...
			<?php 
				...
				while($row = mysqli_fetch_assoc($cat_query_result)){ 
					...
					if (mysqli_num_rows($product_query_result)>0) { ?>
						...
						<div id="cat_head_hover" class="cat_head row ml-1 justify-content-center">
							<h1 id="cat_head_link"><?php echo $row['cat_name']; ?></h1>
						</div>
						...
						<?php }else{  ?>
							<div class=" row col-sm-4 mt-5 justify-content-center" >
								<!-- <button type="button" name="submit" class="btn  col-xl-8 btn-danger mt-3" style="border-radius:20px;">More Item..</button> -->
								<a class="text-info" style="font-size: 30px; text-decoration: none;" href="more.php?cat=<?php echo $row['cat_id']; ?>">Load More...</a>
							</div>
