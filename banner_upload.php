<?php
 ...
 if(isset($_POST['but_upload'])){
  ...
  $viewquery = " SELECT * FROM banner";
  $viewresult = mysqli_query($con,$viewquery);

  if(mysqli_num_rows($viewresult)>0){
      echo '<script>alert("Slide Show Alrady Addedd. You can Only Change Image"); window.location.href="dashboard.php?id=10";</script>';
  }else{
      ...
      if( in_array($imageFileType,$extensions_arr) ){
                 ...
                 $query = "INSERT into banner(image) values('".$name."')";
                 mysqli_query($con,$query);
              
                 // Upload file
                 move_uploaded_file($_FILES['file']['tmp_name'],$target_dir.$name);

                  echo '<script>alert("Slide Show Inserted Success"); window.location.href="dashboard.php?id=10";</script>';
...
<head>
  ...
  <body class="a bg-light">
    ...
    <h3 class=""><a class="text-info ml-5" style="text-decoration: none; float: left; font-size: 30px;" href="image_upload.php"><b>Add Banner </b></a></h3>
